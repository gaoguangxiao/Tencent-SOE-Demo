...
#pragma mark - Init
+ (instancetype)shared {
    static RecordFileHandler *instance = nil;
...
-(void)startVoiceRecordByAudioUnitByAudioConverter:(nullable AudioConverterRef)audioConverter needMagicCookie:(BOOL)isNeedMagicCookie audioDesc:(AudioStreamBasicDescription)audioDesc {
    if (!self.recordFilePath) {
        NSLog(@"%@:%s - record file path is null",kModuleName,__func__);
        return;
    }
...
//    self.recordFilePath = _oralConfig.audioFile;
...
- (NSString *)createFilePath {
    ...
    NSString *fullFileName  = [NSString stringWithFormat:@"%@.wav",date];
